...
import edu.coursera.algs41.TwoStacksQueue
import spock.lang.Specification

class TwoStacksQueueTest extends Specification {
    def 'new queue is empty'() {
        when:
        def queue = new TwoStacksQueue<Integer>()
    ...
    def 'queue created with array is not empty'() {
        when:
        def queue = new TwoStacksQueue<Integer>([1, 2, 3] as Integer[])
    ...
    def 'queue created with iterable is not empty'() {
        when:
        def queue = new TwoStacksQueue<Integer>([1, 2, 3])
